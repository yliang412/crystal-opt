...
__global__ void probe(int *lo_orderdate, int *lo_partkey, int *lo_custkey,
  ...
  // BEGIN bf_p bloom
  BlockPredLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      lo_partkey + tile_offset, items3, num_tile_items, selection_flags);
  BlockProbeBloomFilter<int, BLOCK_THREADS, ITEMS_PER_THREAD>(items3, selection_flags, bf_p, bf_p_size, num_tile_items);
  if (IsTerm<int, BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) { return; }
  // END bf_p bloom

  // BEGIN bf_d bloom
  BlockPredLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      lo_orderdate + tile_offset, items4, num_tile_items, selection_flags);
  BlockProbeBloomFilter<int, BLOCK_THREADS, ITEMS_PER_THREAD>(items4, selection_flags, bf_d, bf_d_size, 19920101, num_tile_items);
  if (IsTerm<int, BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) { return; }
  // END bf_d bloom
  ...
  // BEGIN ht_p hash
  BlockProbeAndPHT_1<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items3, selection_flags, ht_p, p_len, num_tile_items);
  if (IsTerm<int, BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) { return; }
  // END ht_p hash

  // BEGIN ht_d hash
  BlockProbeAndPHT_2<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
      items4, year, selection_flags, ht_d, d_len, 19920101, num_tile_items);
  if (IsTerm<int, BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) { return; }
  // END ht_d hash
...
float runQuery(int *lo_orderdate, int *lo_custkey, int *lo_partkey,
  ...
  build_hashtable_c<128, 4><<<(c_len + tile_items - 1) / tile_items, 128>>>(
      c_region, c_custkey, c_nation, c_len, ht_c, c_len, bf_c, bf_c_size);
  /*CHECK_ERROR();*/
  ...
  build_hashtable_p<128, 4><<<(p_len + tile_items - 1) / tile_items, 128>>>(
      p_mfgr, p_partkey, p_len, ht_p, p_len, bf_p, bf_p_size);
  /*CHECK_ERROR();*/
